...
use cosmwasm_std::{from_binary, to_binary, CosmosMsg, DepsMut, Empty, Response, WasmMsg, StdResult};
...
use crate::{
    ContractError, StakingContract, ExecuteMsg, Extension, InstantiateMsg, MintMsg, QueryMsg,
};
...
fn setup_contract(deps: DepsMut<'_>) -> StakingContract<'static, Extension, Empty> {
    let contract = StakingContract::default();
    let msg = InstantiateMsg {
        launch_owner: CONTRACT_NAME.to_string(),
...
fn proper_instantiation() {
    ...
    let contract = StakingContract::<Extension, Empty>::default();
