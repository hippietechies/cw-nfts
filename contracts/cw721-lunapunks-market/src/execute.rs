...
use crate::msg::{ExecuteMsg, InstantiateMsg, StakingMsg};
use crate::state::{MarketContract};

// version info for migration info
const CONTRACT_NAME: &str = "crates.io:luna-punks-market";
...
impl<'a> MarketContract<'a> {
    ...
    pub fn migrate(
        ...
        //         token.ask = None;
        //         self.token_map.save(deps.storage, U32Key::from(token.token_id), &token);
        //     }
        // }
        ...
        // }
            // .for_each(|x| {
            //     let unwrapped_token = x.ok().unwrap().1;
            //     unwrapped_token.bids.iter().map(|bid| {
            //         self.bid_map.save(deps.storage, (bid.owner.clone(), unwrapped_token.token_id.to_string().as_bytes().to_vec()), &unwrapped_token);
            //         return;
            //     });
            //     return;
            // });
            ...
            // self.state.save(deps.storage, &State {
            //     contract: deps.api.addr_validate("terra1qfy2nfr0zh70jyr3h4ns9rzqx4fl8rxpf09ytv")?,
            //     staking_contract: deps.api.addr_validate("terra14ayqtv6ck5w0u8slpgjp7wkvve9j066aqzgksn")?, // 1% platform fee
            //     owner: deps.api.addr_validate("terra1qzw84hfrha4hjr4q4xsntqduk8lkjmdz2r5deg")?,
            //     platform_fee: 100, // 1% platform fee
            //     platform_wallet: deps.api.addr_validate("terra14ayqtv6ck5w0u8slpgjp7wkvve9j066aqzgksn")?, // 1% platform fee
            //     royalty_fee: 400,
            //     royalty_wallet: deps.api.addr_validate("terra1qzw84hfrha4hjr4q4xsntqduk8lkjmdz2r5deg")?,
            //     royalties: vec![],
            // })?;

            // self.state.update(deps.storage, |mut state| -> Result<_, ContractError> {
            // state.staking_contract = staking_addr.clone();
            // state.platform_wallet = staking_addr;
        //     Ok(state)
        // })?;
